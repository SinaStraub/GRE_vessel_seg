function [vesselness_all,out] = PFT3Dmod(I,tau,tau2,D,whiteondark,scales)
% This is original code from https://github.com/Haifafh/MFAT which was modified
% in lines 1, 39, 79, 88, 92 152, 175 to use the different scales from the 
% Shearlet representation by Sina Straub, sina.straub@gmail.com,sina.straub@dkfz.de
% Date: 27.03.2021 V1.1

%referring to original code: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%start
...
%referring to original code: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%end
%%
verbose = 1;
%% preprocessing 
I = single(I);
...
for j = 1:scales
    ...
    %% (1) Eigen-values
    [~, Lambda2, Lambda3] = volumeEigenvalues(I,whiteondark,j);
...
function [Lambda1, Lambda2, Lambda3] = volumeEigenvalues(V,whiteondark,scales)
% calculates the three eigenvalues for each voxel in a volume

% Calculate 3D hessian
[Hxx, Hyy, Hzz, Hxy, Hxz, Hyz] = Hessian3D(V,scales);
...
function [Dxx, Dyy, Dzz, Dxy, Dxz, Dyz] = Hessian3D(Volume,scales)
